...
- **人机验证（Captcha）**：
  ...
  - **触发模式**：始终显示 / 可疑行为触发 / 高风险操作触发；**显示方式**：内嵌式 / 弹窗式 / 触发式。
...
**Q9. 如何启用推理交换验证？**
在后台「站点设置 → 验证码设置」中选择「推理交换验证（交换图块复原图片）」，并配置触发模式（始终显示/可疑触发/高风险触发）和显示方式（内嵌式/弹窗式/触发式）。该模式支持简中/繁中/英文三语提示。

**Q10. 触发式验证码在哪里设置？**
在后台「站点设置 → 验证码设置」中找到「显示方式」下拉框，选择「触发式（鼠标移入时显示验证框）」。当用户鼠标移动到输入框时会自动弹出验证窗口，用户体验更友好。

...
> **當前狀態：正式版**（`v1.5.4`）｜ 輕量級社區論壇系統 · PHP + SQLite · 開箱即用
...
| 模塊 | 說明 |
| --- | --- |
| 用戶系統 | 註冊、登錄、記住密碼、個人資料、修改密碼、郵箱驗證碼（需 SMTP）、找回密碼、強制改密、封禁申訴；個人中心新增「帳戶資訊」卡片（在線時間 / 註冊時間 / 最後訪問 / 上次活動時間 / 上次發表時間 / 所在時區，時區可自定義） |
| 內容體系 | 多級版塊（分類 → 版塊）、主題帖、回復（樓中樓/引用）、置頂、加精、鎖帖、收藏、搜索、分享（連結自動跟隨當前訪問域名） |
...
| 運維監控 | 流量統計（訪問記錄：PV 精確計數 + 會話級 UV 去重 + 爬蟲過濾 + 地域歸屬，詳見[第 10.4 節](#104-流量監測traffic_monitor)）、IP 庫管理（離線 ip2region 格式，可選安裝：GitHub/國內網盤下載、上傳/查詢/刪除，詳見[第 10.5 節](#105-ip-庫管理-ip_database)）、後台使用者/帖子管理 IP 定位顯示（註冊/活躍/發帖 IP + 歸屬地，詳見[第 10.6 節](#106-使用者管理與帖子的-ip-定位)）、系統狀態、資料庫備份、自動 Schema 遷移、安裝/錯誤日誌 |
...
| 帖子標籤 | 發帖時可添加標籤，帖子詳情頁展示標籤，支援按標籤篩選與熱門標籤雲；標籤寫入時自動去重並統計使用次數 |
| 版塊訂閱 | 用戶可訂閱/取消訂閱版塊；訂閱後該版塊發布新帖時自動通知訂閱者；個人下拉菜單新增「我的訂閱」入口 |
| 帖子編輯 | 作者或擁有 `manage_posts` 權限的管理員可對已發帖進行二次編輯，自動保留編輯歷史（編輯人、編輯時間、編輯說明） |
| 閱讀權限 | 發帖時可設置公開 / 僅登錄會員 / 積分門檻三種閱讀權限；無權限訪問時詳情頁給出對應提示（登錄 / 積分不足） |
| 特殊帖子 | 投票帖（單選/多選、實時結果）、辯論帖（回覆選擇正/反/中立立場並雙欄統計）、懸賞帖（發布即扣除積分，樓主採納最佳答案後發放給回答者） |
...
>
> **安全提醒**：**切勿使用 `777`（世界可寫）權限**。`755` 已足夠——只需將目錄屬主改為 Web 進程用戶即可獲得寫權限，而無需放寬為全局可寫。例如：`chown -R www-data:www-data data uploads`（Apache/Nginx 默認用戶通常為 `www-data`，請按實際環境調整）。
...
| 表 | 作用 |
| --- | --- |
| `users` | 用戶（用戶名/郵箱/密碼/積分/金幣/角色/籤到/封禁禁言狀態；新增 `last_visit`/`timezone`/`online_time` 三列支撐個人中心帳戶資訊） |
...
- **瀏覽**：首頁聚合（統計、公告、版塊、熱門/最新）、版塊帖子列表、主題詳情（含引用回復、分頁）、搜索、按標籤篩選帖子、熱門標籤雲。
- **發布**：發帖、回復（支持 BBCode、表情、引用、圖片上傳）、編輯資料、收藏、帖子分享（一鍵複製當前訪問域名的完整連結）。
- **帖子增強**：
  - **標籤**：發帖/編輯時可添加最多 10 個標籤，用空格或逗號分隔；詳情頁展示標籤並提供熱門標籤雲。
  - **二次編輯**：作者或擁有 `manage_posts` 權限的管理員可編輯已發布帖子；編輯前自動保存舊標題與正文到編輯歷史。
  - **閱讀權限**：發帖時可設置公開 / 僅登錄會員 / 積分門檻三檔；無權限訪問時給出登錄或積分不足提示。
  - **投票帖**：設置投票問題、多個選項，支持單選/多選；用戶投票後實時顯示各選項票數與佔比。
  - **辯論帖**：回復時可選擇正方/反方/中立立場；詳情頁按立場雙欄統計人數。
  - **懸賞帖**：發布時立即扣除懸賞積分；樓主可採納最佳答案，被採納者獲得懸賞積分，帖子同時鎖定。
- **帳戶**：註冊（可選郵箱驗證）、登錄（記住密碼）、找回/重置密碼、強制改密、封禁申訴；個人中心「帳戶資訊」展示在線時間、註冊時間、最後訪問、上次活動、上次發表與所在時區（時區可在編輯資料中設定）。
- **互動**：私信（輪詢實時未讀）、系統通知、每日籤到、積分/金幣與等級展示、勳章牆、版塊訂閱（新帖自動通知訂閱者，「我的訂閱」統一管理）。
...
### 10.6 使用者管理與帖子的 IP 定位

後台使用者管理與帖子管理列表新增 **IP 定位** 列（僅後台可見，前台不展示任何 IP 資訊），便於管理員定位異常註冊與灌水發帖：

- **使用者管理**（`/admin/users`）：每個使用者展示「最後活躍 IP + 歸屬地」（主行）與「註冊 IP + 歸屬地」（小字）。資料來自 `users` 表新增的 `register_ip` / `last_ip` 列：註冊時寫入 `register_ip`，每次登入成功與活躍更新時同步重新整理 `last_ip`（`client_ip()`，`app/includes/functions.php`）。
- **帖子管理**（`/admin/posts`）：每個帖子展示「發帖 IP + 歸屬地」。資料來自 `posts` 表新增的 `ip` 列，發帖時由 `app/controllers/new_post.php` 記錄 `client_ip()`。
- **歸屬地解析**：與流量監測共用 `ip_region_query()` / `ip_region_display()`（`app/includes/ip2region.php`），未安裝 IP 庫時僅顯示 IP、歸屬地留空，不影響其他功能；歷史舊資料（升級前註冊/發帖）IP 列為空時顯示「—」。

> 資料庫升級由 `init_db()` / `ensure_schema_patch()` 自動執行（補丁版本 v9），為 `users` 與 `posts` 表補齊上述列，無需手動操作。

> 資料庫升級由 `init_db()` / `ensure_schema_patch()` 自動執行（補丁版本 v10），為 `users` 表補齊 `last_visit` / `timezone` / `online_time` 三列，支撐個人中心「帳戶資訊」展示；存量庫無需手動操作。
...
**Q1. 安裝時提示「data 目錄不可寫」？**
創建並賦予寫權限：`mkdir data && chmod 755 data`（**請勿使用 `777`**；若目錄已存在，用 `chown -R www-data:www-data data` 將屬主設為 Web 進程用戶即可獲得寫權限。Windows 下在目錄屬性的「安全」中給 Web 進程寫權限）。
...
> 文檔基於項目源碼（`index.php`、`install.php`、`app/includes/*`、`public/*`）整理，版本 `1.5.4`。

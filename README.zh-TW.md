...
| 人機驗證 | 內置「滑塊拼圖」「點選文字」與「推理交換」三種模式人機驗證，支持行為打分、觸發模式（始終顯示/可疑觸發/高風險觸發）與顯示方式（內嵌式/彈窗式/觸發式），GD 生成背景圖，無需第三方服務 |
...
**Q7. 拼圖驗證看起來對齊了但提示失敗？**
請先強制刷新瀏覽器（`Ctrl+F5`）以加載最新 `captcha.js`；若容器被 CSS 壓縮導致舞臺寬度不是 300px，系統會自動按比例換算坐標。也可在後臺「網站設置 → 驗證方式」臨時切到「點選文字」或「推理交換」驗證排查。

**Q8. 點選文字驗證的漢字顯示為方框？**
點選文字依賴 GD 與字體文件渲染。默認使用系統字體，中文顯示不佳時請在 `app/captcha/fonts/` 放置中文字體（如 `SourceHanSansSC-Regular.otf`），系統將自動優先使用。

**Q9. 如何啟用推理交換驗證？**
在後臺「網站設置 → 人機驗證設置」中選擇「推理交換驗證（交換圖塊復原圖片）」，並配置觸發模式（始終顯示/可疑觸發/高風險觸發）和顯示方式（內嵌式/彈窗式/觸發式）。該模式支持簡繁中英文三語提示。

**Q10. 觸發式驗證碼在哪裏設置？**
在後臺「網站設置 → 人機驗證設置」中找到「顯示方式」下拉框，選擇「觸發式（滑鼠移入時顯示驗證框）」。當用戶滑鼠移動到輸入框時會自動彈出驗證視窗，使用者體驗更友好。
